...
// 6번 라인의 해석
//  : localStorage.getItem('todos')에 데이터가 있으면 true이기 때문에 JSON으로 바꿔주고, 아무런 데이터가 없으면 빈 배열을 꺼내라는 의미이다.
...
function renderTodos() {
  ...
  todos.forEach((todo, index) => {
    ...
    if (todo.completed) {
      // localStorage의 'todos' key의 내부를 확인했을 때 배열로 저장되어있고, 거기 내부에 JS 객체가 있는데, 그 객체가 현재 todo라고 이름 붙여져있다. 그 todo들은 text라는 key, completed라는 key를 가지고 있었다.
      ...
      // completed가 true라면 할 일이 완료되었음을 표시해주기 위해서 css 상 차이를 만들었는데, 그걸 적용하기 위한거다.
...
function addTodo() {
  ...
  renderTodos(); // 추가 버튼 누르고 나서 다시 (갱신된) 리스크 가지고 와야 하니까 renderTodos() 함수 호출
